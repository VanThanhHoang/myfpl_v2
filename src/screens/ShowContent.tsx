        ...
        return (
            <View style={{ marginTop: 20, paddingHorizontal: 20 }}>
                {selectedData.map((item) => (
                    <View key={item.id} style={{ backgroundColor: 'white', height: 150, borderRadius: 10, marginBottom: 22, paddingHorizontal: 20, paddingVertical: 10 }}>
                        <Text style={{ fontSize: 18, fontWeight: 'bold', color: 'black' }}>{item.content}</Text>
                ...
                {TABSHOME.map((item) => (
                    <View key={item.id}>
                        <TouchableOpacity style={[styles.tabItem]} onPress={() => setSelectedTabId(item.id)}>
                            ...
                            {item.id === selectedTabId && <View style={styles.tabUnderline} />}
                        </TouchableOpacity>
                    </View>
                ))}
            </View>
