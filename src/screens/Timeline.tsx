...
const OverrideRenderTimeLine: React.FC = () => {
  const data: DataItem[] = [
    {
      time: '09:00',
      timeEnd: '10:30',
  ...
  const renderTime = (rowData: DataItem): JSX.Element => {
    ...
    if (rowData.time && rowData.timeEnd)
      desc = (
        <View>
          <Text style={{ marginHorizontal: 10, fontSize: 16, color: 'black', fontWeight: '700' }}>
            {rowData.time}
          </Text>

          <Text style={{
            marginHorizontal: 10, color: '#BCC1CD',
          }}>
            {rowData.timeEnd}
          </Text>
        </View>
